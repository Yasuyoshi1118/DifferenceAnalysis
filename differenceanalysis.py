...
layout = [
    ...
    sg.Text("確認列："), sg.InputText("12",key="analysis_column",size=(5,1)),
    sg.Text("表示名称列："), sg.InputText("13",key="name_column",size=(5,1))],
    ],
    [
    [sg.Text("解析ファイル選択"), sg.InputText(key="path2"), sg.FileBrowse(key="selected_file_path2"),
    sg.Text("確認列："), sg.InputText("9",key="analysis_column2",size=(5,1))],
    ],
    [
...
def difference_analysis(output_xlsx_name,name_ref_sheet,analysis_col,analysis_col2,tvecsno_check,name_col):
    ...
    for i in list1:
        ...
        if tvecsno_check==1:
            if len(data_i)==5:
                data_i = "0"+ data_i
        print("******************now_analysis tvecsNo.*******************:",data_i)
        check = 0
        for j in list2:
            ...
            if tvecsno_check==1:
                if len(data_j)==5:
                    ...
                    #print("******************data_j*******************:",data_j)
            if(data_i == ""):
                print("==================skip because it's empty=======================")
                check = 1
                break
            print("行:",xy,", answer_data : ",data_i,"    check_data : ",data_j)
            if(data_i == data_j):
                print("******************OK because it exists*******************")
                check = 1
                break
        ...
        if(check==0):
            ...
            print("!!!!!!!!!!問題あり　行；",xy," DATA ; ",data_i,"not exist!!!!!!!!!!!!!")
            #print("check=0")
